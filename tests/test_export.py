...
def get_auth_headers(client: TestClient, user_email: str, password: str = "testpassword123"):
...
def test_export_dataset_access_denied_contributor(client: TestClient, contributor_user, test_data):
...
def test_export_audit_logging(client: TestClient, db_session, sworik_developer_user, test_data):
    ...
    audit_log = db_session.query(ExportAuditLog).order_by(ExportAuditLog.id.desc()).first()
...
def test_export_history_pagination(client: TestClient, sworik_developer_user, test_data):
