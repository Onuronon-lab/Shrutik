...
def get_auth_headers(
    client: TestClient, user_email: str, password: str = "testpass123"
):
...
class TestVoiceRecordingWorkflow:
    ...
    def test_complete_voice_recording_workflow(
        self, client, test_users, test_scripts, test_language
    ):
        ...
        session_data = {"script_id": script_data["id"], "language_id": test_language.id}
    ...
    def test_recording_workflow_with_invalid_script(
        self, client, test_users, test_language
    ):
...
class TestTranscriptionWorkflow:
    ...
    def test_complete_transcription_workflow(
        self, client, db_session, test_users, test_language
    ):
        ...
        task_request = {"quantity": 2, "language_id": test_language.id}
        response = client.post(
            "/api/transcriptions/tasks", json=task_request, headers=headers
        )
        assert response.status_code == 200
    ...
    def test_transcription_skip_functionality(
        self, client, db_session, test_users, test_language
    ):
        ...
        response = client.post(
            "/api/transcriptions/skip", json=skip_data, headers=headers
        )
        assert response.status_code == 200
    ...
    def test_transcription_workflow_validation(self, client, test_users):
        ...
        submission_data = {
            ...
            "transcriptions": [
                {"chunk_id": 1, "text": "", "confidence": 0.9}  # Empty text
            ],
        }
        response = client.post(
            "/api/transcriptions/submit", json=submission_data, headers=headers
...
class TestAdminWorkflow:
    ...
    def test_quality_review_workflow(
        self, client, db_session, test_users, test_language
    ):
...
class TestExportWorkflow:
    ...
    def test_complete_export_workflow(
        ...
        response = client.post(
            "/api/export/dataset", json=export_request, headers=sworik_headers
        )
        assert response.status_code == 200
        ...
        response = client.post(
            "/api/export/metadata", json=metadata_request, headers=sworik_headers
        )
        assert response.status_code == 200
        ...
        audit_logs = (
            db_session.query(ExportAuditLog).filter_by(user_id=sworik_dev.id).all()
        )
    ...
    def test_export_access_control(self, client, db_session, test_users, test_language):
        ...
        response = client.post(
            "/api/export/dataset", json=export_request, headers=contributor_headers
        )
        assert response.status_code == 403
        ...
        response = client.post(
            "/api/export/dataset", json=export_request, headers=admin_headers
        )
        assert response.status_code == 403
    ...
    def test_export_with_various_filters(
        ...
        export_request = {
            "format": "json",
            "quality_filters": {"min_quality": 0.9, "min_confidence": 0.85},
        }
        response = client.post(
            "/api/export/dataset", json=export_request, headers=sworik_headers
        )
        ...
        export_request = {"format": "csv", "language_ids": [test_language.id]}
        response = client.post(
            "/api/export/dataset", json=export_request, headers=sworik_headers
        )
        assert response.status_code == 200
        ...
        export_request = {"format": "json", "max_records": 5}
        response = client.post(
            "/api/export/dataset", json=export_request, headers=sworik_headers
        )
        assert response.status_code == 200
...
class TestWorkflowIntegration:
    ...
    def test_full_platform_workflow(
        self, client, db_session, test_users, test_language, test_scripts
    ):
        ...
        task_request = {"quantity": 2}
        response = client.post(
            "/api/transcriptions/tasks", json=task_request, headers=contributor_headers
        )
        assert response.status_code == 200
        task_data = response.json()
        ...
        export_request = {"format": "json", "quality_filters": {"validated_only": True}}
        response = client.post(
            "/api/export/dataset", json=export_request, headers=sworik_headers
        )
        assert response.status_code == 200
    ...
    def test_error_handling_across_workflows(self, client, test_users):
        ...
        response = client.post(
            "/api/transcriptions/submit",
            json={"session_id": "non_existent_session", "transcriptions": []},
            headers=headers,
        )
        ...
        response = client.post(
            ...
            json={"quantity": 999},  # Invalid quantity
            headers=headers,
        )
