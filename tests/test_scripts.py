...
def test_create_script_contributor_forbidden(client, contributor_user, sample_script_data):
...
def test_get_random_script_success(client, admin_user, contributor_user, sample_script_data):
    ...
    headers = {"Authorization": f"Bearer {contributor_user}"}
    response = client.get("/api/scripts/random?duration_category=2_minutes", headers=headers)
...
def test_get_random_script_not_found(client, contributor_user, test_language):
    ...
    headers = {"Authorization": f"Bearer {contributor_user}"}
    response = client.get(f"/api/scripts/random?duration_category=5_minutes&language_id={test_language.id}", headers=headers)
...
def test_get_script_by_id(client, admin_user, sample_script_data):
    ...
    # Create script
    create_response = client.post("/api/scripts/", json=sample_script_data, headers=headers)
...
def test_update_script(client, admin_user, sample_script_data):
    ...
    update_data = {"text": "আপডেট করা স্ক্রিপ্ট টেক্সট যা পরীক্ষার জন্য ব্যবহৃত হচ্ছে। এটি একটি দীর্ঘ টেক্সট যা যথেষ্ট শব্দ রয়েছে। এই স্ক্রিপ্টটি দুই মিনিটের জন্য উপযুক্ত। এতে পর্যাপ্ত শব্দ রয়েছে যা একজন ব্যক্তি দুই মিনিটে পড়তে পারবেন।"}
    response = client.put(f"/api/scripts/{script_id}", json=update_data, headers=headers)
