...
class NotificationService:
    ...
    def send_notification(
        ...
        for channel in channels:
            try:
                ...
                logger.error(
                    f"Failed to send notification {notification.id} via {channel.value}: {e}"
                )
            ...
            for data in notifications_data:
                try:
                    ...
                    if notification_dict.get("id") == notification_id:
                        notification_dict["read_at"] = datetime.now(
                            timezone.utc
                        ).isoformat()
                        found = True
    ...
    def _send_database_notification(self, notification: Notification):
        ...
        logger.info(
            f"Database notification: {notification.title} - {notification.message}"
        )
