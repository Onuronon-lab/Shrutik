...
async def get_random_script(
    duration_category: DurationCategory = Query(
        ..., description="Duration category for the script"
    ),
    language_id: Optional[int] = Query(
        None, description="Language ID (optional, defaults to Bangla)"
    ),
...
async def list_scripts(
    ...
    limit: int = Query(
        100, ge=1, le=1000, description="Maximum number of scripts to return"
    ),
    duration_category: Optional[DurationCategory] = Query(
        None, description="Filter by duration category"
    ),
