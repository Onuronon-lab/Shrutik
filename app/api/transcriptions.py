...
@router.post(
    "/tasks", response_model=TranscriptionTaskResponse, status_code=status.HTTP_200_OK
)
async def get_transcription_task(
    ...
    return transcription_service.get_random_chunks_for_transcription(
        current_user.id, task_request
    )


@router.post(
    "/submit",
    response_model=TranscriptionSubmissionResponse,
...
async def get_user_transcriptions(
    ...
    limit: int = Query(
        100, ge=1, le=1000, description="Maximum number of transcriptions to return"
    ),
    ...
    return transcription_service.get_user_transcriptions(
        user_id=current_user.id, skip=skip, limit=limit, language_id=language_id
    )
...
async def get_transcription(
    ...
    user_id = (
        current_user.id
        if current_user.role not in ["admin", "sworik_developer"]
        else None
    )

    transcription = transcription_service.get_transcription_by_id(
...
async def update_transcription(
    ...
    transcription = transcription_service.update_transcription(
        transcription_id=transcription_id, update_data=update_data, user_id=user_id
    )
...
async def get_all_transcriptions(
    ...
    language_id: Optional[int] = Query(None, description="Filter by language ID"),
    is_consensus: Optional[bool] = Query(
        None, description="Filter by consensus status"
    ),
...
async def get_transcription_statistics(
    current_user: User = Depends(require_admin_or_sworik), db: Session = Depends(get_db)
):
    """
...
async def get_problematic_chunks(
    min_skips: int = Query(
        3, ge=1, description="Minimum number of skips to consider problematic"
    ),
    ...
    query = (
        db.query(AudioChunk)
        .filter(text("JSON_LENGTH(meta_data->'$.skips') >= :min_skips"))
    ...
    for chunk in chunks:
        ...
        transcription_count = (
            db.query(Transcription).filter(Transcription.chunk_id == chunk.id).count()
        )
