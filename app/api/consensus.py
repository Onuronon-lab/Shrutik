...
from app.core.dependencies import get_current_user, get_db, require_admin_or_sworik
...
class ManualReviewRequest(BaseModel):
    ...
    selected_transcription_id: Optional[int] = Field(
        None, description="ID of transcription to mark as consensus"
    )
...
async def trigger_batch_consensus_calculation(
    chunk_ids: List[int], current_user: User = Depends(require_admin_or_sworik)
):
    """
...
async def trigger_full_consensus_recalculation(
    ...
    try:
        ...
        return {"message": "Full consensus recalculation queued", "task_id": task.id}
...
async def get_chunks_requiring_review(
    limit: int = Query(
        50, ge=1, le=100, description="Maximum number of chunks to return"
    ),
...
async def get_chunk_validation_status(
    ...
    try:
        ...
        transcriptions = (
            db.query(Transcription).filter(Transcription.chunk_id == chunk_id).all()
        )
        ...
        for transcription in transcriptions:
            if (
                ...
                and transcription.meta_data["consensus_evaluation"].get(
                    "requires_review", False
                )
            ):
