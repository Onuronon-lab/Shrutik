...
class JobActionRequest(BaseModel):
    ...
    countdown: Optional[int] = Field(60, description="Delay in seconds for retry action")
...
async def perform_job_action(
    ...
    if action == "cancel":
        ...
        new_task_id = job_monitoring_service.retry_job(task_id, countdown=action_request.countdown)
        return {
            "action": "retry",
            "original_task_id": task_id,
            "new_task_id": new_task_id,
            "success": new_task_id is not None,
            "message": f"Job retried with ID {new_task_id}" if new_task_id else "Failed to retry job"
        }
...
async def purge_queue(
    purge_request: QueuePurgeRequest,
    current_user: User = Depends(require_admin)
) -> Dict[str, Any]:
    """
