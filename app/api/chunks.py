...
async def get_chunk_audio(
    ...
    if "error" in optimization:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=optimization["error"]
        )
    ...
    if cdn_url and not cdn_url.startswith("/api/"):
        ...
        return RedirectResponse(
            url=cdn_url, status_code=status.HTTP_302_FOUND, headers=headers
        )

    chunk = db.query(AudioChunk).filter(AudioChunk.id == chunk_id).first()
    if not chunk:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Audio chunk not found"
        )
...
async def get_chunk_info(
    ...
    transcription_count = (
        db.query(Transcription).filter(Transcription.chunk_id == chunk_id).count()
    )
