...
async def upload_recording(
    ...
    audio_format: str = Form(
        ..., description="Audio format (wav, mp3, m4a, flac, webm)"
    ),
...
async def get_user_recordings(
    ...
    limit: int = Query(
        100, ge=1, le=1000, description="Maximum number of recordings to return"
    ),
    status: Optional[RecordingStatus] = Query(
        None, description="Filter by recording status"
    ),
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db),
):
...
async def get_all_recordings(
    ...
    if user_id:
        ...
        return VoiceRecordingListResponse(
            recordings=[], total=0, page=1, per_page=limit, total_pages=0
        )
...
async def batch_process_recordings(
    ...
    if not valid_recordings:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No valid recordings found"
        )
...
async def reprocess_failed_recordings(
    ...
    return {"message": "Failed recordings reprocessing triggered", "task_id": task.id}
...
async def cleanup_orphaned_chunks(
    ...
    return {"message": "Orphaned chunks cleanup triggered", "task_id": task.id}
