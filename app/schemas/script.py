...
class ScriptBase(BaseModel):
    ...
    text: str = Field(..., min_length=1, max_length=10000, description="Script text content")
    duration_category: DurationCategory = Field(..., description="Duration category for the script")
...
class RandomScriptRequest(BaseModel):
    ...
    duration_category: DurationCategory = Field(..., description="Desired duration category")
    language_id: Optional[int] = Field(None, gt=0, description="Language ID (optional, defaults to Bangla)")
...
class ScriptValidationResponse(BaseModel):
    ...
    estimated_duration: Optional[float] = Field(None, description="Estimated reading duration in minutes")
    word_count: int
    character_count: int