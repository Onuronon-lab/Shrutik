...
class ConsensusEvaluationRequest(BaseModel):
    ...
    force_recalculation: bool = Field(False, description="Force recalculation even if already evaluated")
...
class ValidationStatistics(BaseModel):
    ...
    validation_rate: float = Field(..., ge=0.0, le=100.0, description="Percentage of validated chunks")
...
class ReviewQueueFilter(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                "flagged_reasons": ["Low consensus confidence", "Large transcription length differences"],
