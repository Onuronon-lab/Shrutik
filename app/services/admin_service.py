...
from app.schemas.admin import (
    ...
    QualityReviewItemResponse, FlaggedTranscriptionResponse,
    SystemHealthResponse, UsageAnalyticsResponse
)
...
class AdminService:
    ...
    def get_platform_statistics(self) -> PlatformStatsResponse:
        ...
        avg_recording_duration = self.db.query(func.avg(VoiceRecording.duration)).scalar()
        
        # Recording status breakdown
        recording_status_counts = self.db.query(
            VoiceRecording.status, func.count(VoiceRecording.id)
        ...
        total_validated = self.db.query(func.count(Transcription.id)).filter(
            Transcription.is_validated == True
        ).scalar() or 0
        
        # Transcription validation breakdown
        validated_count = self.db.query(func.count(Transcription.id)).filter(
            Transcription.is_validated == True
        ).scalar() or 0
        ...
        # Quality statistics
        total_quality_reviews = self.db.query(func.count(QualityReview.id)).scalar() or 0
        avg_transcription_quality = self.db.query(func.avg(Transcription.quality)).scalar()
    ...
    def get_user_statistics(self, limit: int = 50) -> List[UserStatsResponse]:
        ...
        users_with_stats = self.db.query(
            User,
    ...
    def get_users_for_management(self, role_filter: Optional[UserRole] = None) -> List[UserManagementResponse]:
        ...
        for user in users:
            ...
            recordings_count = self.db.query(func.count(VoiceRecording.id)).filter(
                VoiceRecording.user_id == user.id
            ).scalar() or 0
            
            transcriptions_count = self.db.query(func.count(Transcription.id)).filter(
                Transcription.user_id == user.id
            ).scalar() or 0
            
            quality_reviews_count = self.db.query(func.count(QualityReview.id)).filter(
                QualityReview.reviewer_id == user.id
            ).scalar() or 0
            
            # Get last activity (most recent recording or transcription)
            last_recording = self.db.query(func.max(VoiceRecording.created_at)).filter(
                VoiceRecording.user_id == user.id
            ).scalar()
            
            last_transcription = self.db.query(func.max(Transcription.created_at)).filter(
                Transcription.user_id == user.id
            ).scalar()
    ...
    def get_quality_review_items(self, limit: int = 50) -> List[QualityReviewItemResponse]:
        ...
        result = []
        for review, text, chunk_id, contributor_name, contributor_id, file_path, reviewer_name in reviews:
            result.append(QualityReviewItemResponse(
                id=review.id,
    ...
    def get_flagged_transcriptions(self, limit: int = 50) -> List[FlaggedTranscriptionResponse]:
        ...
        flagged = self.db.query(
         ...
         .join(AudioChunk, Transcription.chunk_id == AudioChunk.id)\
         .outerjoin(QualityReview, Transcription.id == QualityReview.transcription_id)\
         .filter(
            or_(
        ...
        for transcription, contributor_name, file_path, review_count in flagged:
            result.append(FlaggedTranscriptionResponse(
    ...
    def create_quality_review(self, transcription_id: int, reviewer_id: int, 
                            decision: ReviewDecision, rating: Optional[float] = None, 
                            comment: Optional[str] = None) -> QualityReview:
    ...
    def get_system_health(self) -> SystemHealthResponse:
        ...
        active_24h = self.db.query(func.count(func.distinct(User.id))).join(
            VoiceRecording, User.id == VoiceRecording.user_id
        ).filter(VoiceRecording.created_at >= day_ago).scalar() or 0
        
        active_7d = self.db.query(func.count(func.distinct(User.id))).join(
            VoiceRecording, User.id == VoiceRecording.user_id
        ).filter(VoiceRecording.created_at >= week_ago).scalar() or 0

        # Failed recordings count
        failed_recordings = self.db.query(func.count(VoiceRecording.id)).filter(
            VoiceRecording.status == RecordingStatus.FAILED
        ).scalar() or 0

        # Processing queue size (recordings in processing state)
        processing_queue = self.db.query(func.count(VoiceRecording.id)).filter(
            VoiceRecording.status == RecordingStatus.PROCESSING
        ).scalar() or 0
    ...
    def get_usage_analytics(self, days: int = 30) -> UsageAnalyticsResponse:
        ...
        script_popularity = self.db.query(
            Script.duration_category,
            func.count(VoiceRecording.id).label('count')
        ).join(VoiceRecording, Script.id == VoiceRecording.script_id)\
         .filter(VoiceRecording.created_at >= start_date)\
         .group_by(Script.duration_category).all()

        # Top contributors
        top_contributors = self.db.query(
            User.id,
            User.name,
            (func.count(VoiceRecording.id) + func.count(Transcription.id)).label('contribution_count')
        ).outerjoin(VoiceRecording, User.id == VoiceRecording.user_id)\
         .outerjoin(Transcription, User.id == Transcription.user_id)\
        ...
        return UsageAnalyticsResponse(
            daily_recordings=[{"date": str(date), "count": count} for date, count in daily_recordings],
            daily_transcriptions=[{"date": str(date), "count": count} for date, count in daily_transcriptions],
            user_activity_by_role={role.value: count for role, count in user_activity},
            popular_script_durations={duration.value: count for duration, count in script_popularity},
