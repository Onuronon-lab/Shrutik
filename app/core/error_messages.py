...
def get_user_friendly_error(
    ...
    if details and error_code == "FILE_TOO_LARGE":
        ...
        result["message"] = (
            f"Your file is too large to upload. The maximum size is {max_size_mb}MB."
        )

    elif details and error_code == "UNSUPPORTED_FORMAT":
        provided_format = details.get("provided_format", "unknown")
        result["message"] = (
            f"The format '{provided_format}' isn't supported. Please use WAV, MP3, M4A, FLAC, or WebM format."
        )

    elif details and error_code == "AUDIO_TOO_SHORT":
        min_duration = details.get("min_duration", 1)
        result["message"] = (
            f"Your recording is too short. Please record for at least {min_duration} seconds."
        )
        ...
        result["message"] = (
            f"Your recording is too long. Please keep recordings under {max_minutes} minutes."
        )
...
def get_fallback_response(operation: str) -> Dict[str, Any]:
    ...
    return fallbacks.get(
        operation,
        {
            ...
            "alternatives": [
                {"action": "retry", "description": "Try again", "immediate": True},
                {
                    "action": "contact_support",
                    "description": "Contact support for help",
                    "immediate": False,
...
def create_error_response_with_fallback(
    error_code: str, operation: str, details: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
