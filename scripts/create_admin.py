...
def create_admin_user(
    name: str, email: str, password: str, role: UserRole = UserRole.ADMIN
) -> bool:
...
def main():
    ...
    if os.getenv("ENVIRONMENT") == "production":
        confirm = input(
            "⚠️  WARNING: Running in PRODUCTION environment. Continue? (yes/no): "
        )
...
Examples:
  ...
  python scripts/create_admin.py --name "Developer" --email dev@example.com --role sworik_developer
    ...
    parser.add_argument(
        "--password", help="Password (if not provided, will prompt securely)"
    )
    parser.add_argument(
        "--role",
