...
from app.tasks.audio_processing import process_audio_recording, batch_process_recordings
...
def test_chunking_algorithm(audio_file_path: str, output_dir: Optional[str] = None) -> None:
    ...
    try:
        ...
        # Create chunks
        chunk_intervals = chunking_service.create_chunks_intelligent(audio_data, sr, boundaries)
        logger.info(f"Created {len(chunk_intervals)} chunks")
        
        # Display chunk information
        for i, (start, end) in enumerate(chunk_intervals):
            duration = end - start
            logger.info(f"Chunk {i}: {start:.2f}s - {end:.2f}s (duration: {duration:.2f}s)")
...
def process_recording_by_id(recording_id: int) -> None:
    ...
    try:
        ...
        chunking_service = AudioChunkingService()
        chunks = chunking_service.process_recording(recording_id, recording.file_path, db)
        
        logger.info(f"Successfully processed recording {recording_id} into {len(chunks)} chunks")
        
        # Display chunk information
        for chunk in chunks:
            logger.info(f"Chunk {chunk.chunk_index}: {chunk.start_time:.2f}s - {chunk.end_time:.2f}s "
...
def process_all_uploaded_recordings() -> None:
    ...
    try:
        ...
        uploaded_recordings = db.query(VoiceRecording).filter(
            VoiceRecording.status == RecordingStatus.UPLOADED
        ).all()
...
def show_recording_statistics() -> None:
    ...
    try:
        ...
        uploaded_count = db.query(VoiceRecording).filter(
            VoiceRecording.status == RecordingStatus.UPLOADED
        ).count()
        processing_count = db.query(VoiceRecording).filter(
            VoiceRecording.status == RecordingStatus.PROCESSING
        ).count()
        chunked_count = db.query(VoiceRecording).filter(
            VoiceRecording.status == RecordingStatus.CHUNKED
        ).count()
        failed_count = db.query(VoiceRecording).filter(
...
def main():
    ...
    test_parser = subparsers.add_parser('test-chunking', help='Test chunking algorithm on an audio file')
    ...
    process_parser = subparsers.add_parser('process-recording', help='Process a specific recording by ID')
    process_parser.add_argument('recording_id', type=int, help='Recording ID to process')
    
    # Process all uploaded command
    subparsers.add_parser('process-all-uploaded', help='Process all recordings with UPLOADED status')
    
    # Statistics command
    subparsers.add_parser('stats', help='Show recording and chunk statistics')
    
    # Cleanup command
    subparsers.add_parser('cleanup-failed', help='Reset failed recordings to uploaded status')
