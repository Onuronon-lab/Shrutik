...
def upgrade() -> None:
    ...
    op.create_table('users',
        ...
        sa.Column('role', sa.Enum('CONTRIBUTOR', 'ADMIN', 'SWORIK_DEVELOPER', name='userrole'), nullable=False),
    ...
    op.create_table('scripts',
        ...
        sa.Column('duration_category', sa.Enum('SHORT', 'MEDIUM', 'LONG', name='durationcategory'), nullable=False),
    ...
    op.create_index(op.f('ix_scripts_duration_category'), 'scripts', ['duration_category'], unique=False)
    ...
    op.create_table('voice_recordings',
        ...
        sa.Column('status', sa.Enum('UPLOADED', 'PROCESSING', 'CHUNKED', 'FAILED', name='recordingstatus'), nullable=False),
        sa.Column('meta_data', sa.JSON(), nullable=True),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.ForeignKeyConstraint(['language_id'], ['languages.id'], ),
    ...
    op.create_index(op.f('ix_voice_recordings_language_id'), 'voice_recordings', ['language_id'], unique=False)
    op.create_index(op.f('ix_voice_recordings_script_id'), 'voice_recordings', ['script_id'], unique=False)
    ...
    op.create_table('transcriptions',
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.ForeignKeyConstraint(['chunk_id'], ['audio_chunks.id'], ),
        sa.ForeignKeyConstraint(['language_id'], ['languages.id'], ),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_transcriptions_chunk_id'), 'transcriptions', ['chunk_id'], unique=False)
    op.create_index(op.f('ix_transcriptions_id'), 'transcriptions', ['id'], unique=False)
    op.create_index(op.f('ix_transcriptions_is_consensus'), 'transcriptions', ['is_consensus'], unique=False)
    ...
    op.create_table('quality_reviews',
        ...
        sa.Column('decision', sa.Enum('APPROVED', 'REJECTED', 'NEEDS_REVISION', 'FLAGGED', name='reviewdecision'), nullable=False),
    ...
    op.create_index(op.f('ix_quality_reviews_decision'), 'quality_reviews', ['decision'], unique=False)
    op.create_index(op.f('ix_quality_reviews_id'), 'quality_reviews', ['id'], unique=False)
    ...
    op.create_index('ix_transcriptions_user_created', 'transcriptions', ['user_id', 'created_at'])
    op.create_index('ix_quality_reviews_transcription_decision', 'quality_reviews', ['transcription_id', 'decision'])
